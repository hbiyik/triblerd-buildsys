...
class LibtorrentRecipe(Recipe):
    ...
    '''
    version = packages["libtorrent"][0].replace(".", "_")
    url = 'https://github.com/arvidn/libtorrent/archive/libtorrent-{version}.tar.gz'

    depends = ['boost']
    opt_depends = ['openssl']
    patches = ["lt_115.patch"]

    # libtorrent.so is not included because is not a system library
    generated_libraries = [
        'boos_chrono', 'boost_random', 'boost_system', 'boost_python{py_version}', 'torrent_rasterbar']
    ...
    def build_arch(self, arch):
        ...
        b2_build_dir = (
            'build/clang-linux-{arch}/release/abi-aapcs/boost-link-shared/{encryption}/'
            'target-os-android/threading-multi/'.format(
